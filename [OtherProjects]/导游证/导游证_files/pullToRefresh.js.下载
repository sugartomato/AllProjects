...
var refresher = {
	...
	init: function(parameter) {
		if ( parameter.pullUpLableTip ) {
			refresher.info.pullUpLable = parameter.pullUpLableTip;
			refresher.info.pullingUpLable = parameter.pullUpLableTip;
			refresher.info.loadingLable = parameter.pullUpLableTip;
		}
	...
	onRelease: function(pullDownEl, pullUpEl) {
		...
		if (pullUpEl.className.match('loading')) {
			...
			// setTimeout(function(){
				// document.querySelector('.scroller').style.cssText = 'transition-property: transform; transform-origin: 0px 0px 0px; transition-timing-function: cubic-bezier(0.33, 0.66, 0.66, 1); transform: translate(0px, -40px) scale(1) translateZ(0px); transition-duration: 0ms;';
				// var src2_1 = document.getElementById('src2_1');
				// if ( src2_1 && src2_1.style.visibility != 'visible' ) {
					// var src2_2 = document.getElementById('src2_2');
					// src2_1.style.visibility = 'visible';
					// src2_2.style.visibility = 'visible';
				// }
			// }, 50 );
		}
	},
